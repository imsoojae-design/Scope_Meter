            ...
            try {
                ...
                if (n > 0) {
                    ...
                    if (end > 0) {
                        ...
                        // 완성된 프레임 한 줄 로그
                        String frameHex = MeterProtocol.toHex(frame);
                        addLog("← " + end + "B: " + frameHex, "HEX");

                        MeterProtocol.ParseResult r =
                            MeterProtocol.parseLongFrame(frame);
                        final byte[] frameCopy = frame;
                        mainHandler.post(() -> {
                            // RX 파형 + HEX 표시
                            if (readFragment != null)
                                readFragment.showRxWave(frameCopy, frameHex);
                            handleResult(r);
                        });
                    }
    ...
    public void sendRequest(int addr) {
        ...
        writeExecutor.execute(() -> {
            ...
            try {
                ...
                // TX 파형 표시
                mainHandler.post(() -> {
                    if (readFragment != null) readFragment.showTxWave(frame);
                });
